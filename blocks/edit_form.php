...
class block_rucksack_edit_form extends block_edit_form {
    ...
    /**
     * Build a custom sub-accordion section containing a textarea + reset button.
     *
     * Moodle's form API renders every 'header' at the top DOM level, so native
     * nesting is impossible.  We use raw html elements as open/close wrappers
     * and place the real form fields inside them.  That way the whole block
     * hides when the parent "Template set" accordion is collapsed.
     *
     ...
     * @param int $rows
     */
    protected function add_sub_accordion_textarea($mform, $fieldname, $label, $helpstring, $currentcontent, $defaultcontent, $rows = 20) {
        $divid = 'rucksack-sub-' . str_replace('_', '-', $fieldname);

        // Open wrapper.
        $mform->addElement('html', '<div class="rucksack-sub-section" style="margin-left:2em;margin-top:1em;">');

        // Toggle button (inline JS toggles the sibling div).
        $togglejs = 'var d=document.getElementById(' . json_encode($divid) . ');'
            . 'if(d){d.style.display=d.style.display===\'none\'?\'block\':\'none\';}'
            . 'var b=this.querySelector(\'i\');if(b){b.classList.toggle(\'fa-chevron-right\');b.classList.toggle(\'fa-chevron-down\');}';
        $togglebtn = html_writer::tag('button',
            '<i class="icon fa fa-chevron-right"></i> ' . s($label),
            [
                'type' => 'button',
                'class' => 'btn btn-link ftoggler',
                'onclick' => $togglejs,
            ]
        );
        $mform->addElement('static', $fieldname . '_toggle', '', $togglebtn);

        // Content container.
        $mform->addElement('html', '<div id="' . $divid . '" style="display:none;margin-top:0.5em;">');

        // Reset button.
        $defaultb64 = base64_encode($defaultcontent);
        $resetjs = 'var t=document.querySelector(' . json_encode('textarea[name="' . $fieldname . '"]:not([type="hidden"])') . ');'
    ...
    protected function specific_definition($mform) {
        ...
        // Main template set header (accordion).
        $mform->addElement('header', 'templateset_main', get_string('templatesets', 'block_rucksack'));
        ...
        // Sub-accordions inside the template set section.
        $this->add_sub_accordion_textarea(
            $mform,
            'config_template',
        ...
        // Competency sub-accordion (custom HTML wrapper).
        $compdivid = 'rucksack-sub-competencies';
        $mform->addElement('html', '<div class="rucksack-sub-section" style="margin-left:2em;margin-top:1em;">');

        $comptogglejs = 'var d=document.getElementById(' . json_encode($compdivid) . ');'
            . 'if(d){d.style.display=d.style.display===\'none\'?\'block\':\'none\';}'
            . 'var b=this.querySelector(\'i\');if(b){b.classList.toggle(\'fa-chevron-right\');b.classList.toggle(\'fa-chevron-down\');}';
        $comptoggle = html_writer::tag('button',
            '<i class="icon fa fa-chevron-right"></i> ' . s(get_string('competenciesvisibility', 'block_rucksack')),
            [
                'type' => 'button',
                'class' => 'btn btn-link ftoggler',
                'onclick' => $comptogglejs,
            ]
        );
        $mform->addElement('static', 'competency_toggle', '', $comptoggle);
        $mform->addElement('html', '<div id="' . $compdivid . '" style="display:none;margin-top:0.5em;">');
        ...
        $initjs = 'var treeUrl=' . $treeurl . ';var visUrl=' . $visurl . ';var sortUrl=' . $sorturl . ';var sesskey=' . $sesskey . ';'
            . 'window.rucksackGetSetId=function(){var dd=document.querySelector(\'select[name="config_templateset"]\');return dd?parseInt(dd.value,10):0;};'
            . 'window.rucksackLoadCompTree=function(parentid,container){'
            . 'var setid=window.rucksackGetSetId();'
            ...
            . 'else{alert("Sortierfehler: "+(d.error||"Unbekannter Fehler"));}})'
            . '.catch(function(e){alert("Netzwerkfehler beim Sortieren");});};'
        ...
        $mform->addHelpButton('competency_tree', 'competenciesvisibility', 'block_rucksack');
        ...
        // Inject CSS to make the block edit modal wider.
        $modalcss = '<style>'
            . '.modal-dialog { max-width: 95vw !important; min-width: 800px !important; }'
            . '.modal-content { min-width: 800px !important; }'
            . '</style>';
        $mform->addElement('html', $modalcss);
    }
}
